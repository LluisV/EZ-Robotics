...
class PluginAPI {
  constructor(pluginId, permissions = []) {
    ...
    // Initialize all API objects in constructor to make them writable
    this.serial = this._createSerialAPI();
    this.position = this._createPositionAPI();
    this.gcode = this._createGCodeAPI();
    this.status = this._createStatusAPI();
    this.console = this._createConsoleAPI();
    this.ui = this._createUIAPI();
    this.storage = this._createStorageAPI();
    this.python = this._createPythonAPI();
  }

  /**
   * Create Serial Communication API
   * @private
   */
  _createSerialAPI() {
    return {
  ...
  _createPythonAPI() {
    return {
       ...
       */
      connect: async () => {
        // This will be overridden by PluginManager with actual implementation
        throw new Error('Python support not initialized');
      },

      /**
       * Execute a Python function
       * @param {string} functionName - Name of the Python function to execute
       * @param {Array} args - Arguments to pass to the function
       * @param {Object} kwargs - Keyword arguments to pass
      ...
      /**
       * Start streaming data from Python
       * @param {string} generatorName - Name of the Python async generator
       * @param {Array} args - Arguments to pass
       * @param {Object} kwargs - Keyword arguments
       * @param {Function} callback - Callback for streamed data (data, error)
       * @returns {Promise<Function>} Stop function
       */
      startStream: async (generatorName, args = [], kwargs = {}, callback) => {
        // This will be overridden by PluginManager with actual implementation
        throw new Error('Python support not initialized');
      }
    };
